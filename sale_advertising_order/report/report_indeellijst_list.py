    ...
    def generate_xlsx_report(self, workbook, data, orderLines):
        def _form_data(sol):
            ...
            if sol.product_id.width and sol.product_id.height:
                pro_name += (
                    ", "
                    + str(sol.product_id.width)
                    + "x"
                    + str(sol.product_id.height)
                    + "mm"
                )
        ...
        line_count = orderLines.read_group(
            [("id", "in", orderLines.ids)],
            ["title", "adv_issue"],
            ["title", "adv_issue"],
        )
        if len(line_count) > 1:
            raise UserError(
                _(
                    "Selected records must be identical to 'Title' and 'Advertising Issue'!"
                )
            )

        orderByAdClass = orderLines.read_group(
            [("id", "in", orderLines.ids)], ["ad_class"], ["ad_class"]
        )
