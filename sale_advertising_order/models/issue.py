...
class AdvertisingIssue(models.Model):
    ...
    def _get_attribute_domain(self):
        _id = self.env.ref("sale_advertising_order.attribute_title").id
        return [("attribute_id", "=", _id)]
    ...
    def _compute_medium_domain(self):
        ...
        for rec in self:
            ...
            if rec.parent_id:
                ...
                domain = json.dumps([("parent_id", "child_of", i2p)])
    ...
    child_ids = fields.One2many(
        "sale.advertising.issue", "parent_id", "Issues", index=True
    )
    parent_id = fields.Many2one(
        "sale.advertising.issue", "Title", index=True, ondelete="cascade"
    )
    product_attribute_value_id = fields.Many2one(
        "product.attribute.value", string="Variant Title", domain=_get_attribute_domain
    )
    analytic_account_id = fields.Many2one(
        "account.analytic.account",
        required=True,
        string="Related Analytic Account",
        ondelete="restrict",
        help="Analytic-related data of the issue",
    )
    issue_date = fields.Date()
    issue_week_number = fields.Integer(
        string="Week Number", store=True, readonly=True, compute="_compute_week_number"
    )
    week_number_even = fields.Boolean(
        string="Even Week Number",
        store=True,
        readonly=True,
        compute="_compute_week_number",
    )
    deadline = fields.Datetime(help="Closing Time for Sales")
    medium_domain = fields.Char(
        compute="_compute_medium_domain",
        readonly=True,
        store=False,
    )
    state = fields.Selection(
        [("open", "Open"), ("close", "Close")], "Status", default="open"
    )
    ...
    def action_open_issue(self):
        action = self.env["ir.actions.actions"]._for_xml_id(
            "sale_advertising_order.action_sale_advertising_issue_title"
        )
