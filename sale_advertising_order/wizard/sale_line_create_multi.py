...
class sale_order_line_create_multi_lines(models.TransientModel):
    ...
    def create_multi_lines(self, raise_exception=True):
        ...
        if model and model == "sale.order":
            ...
            if n == 0:
                ...
                return self.create_multi_from_order_lines(
                    orderlines=all_lines, orders=order_ids
                )

        elif model and model == "sale.order.line":
            line_ids = context.get("active_ids", [])
            all_lines = self.env["sale.order.line"].search(
                [("id", "in", line_ids), ("multi_line", "=", True)]
            )
            if not all_lines:
                raise UserError(
                    _(
                        "There are no Sales Order Lines with "
                        "Multi Lines in the selection."
                    )
                )
            orders = self.env["sale.order"].search(
                [("id", "in", all_lines.mapped("order_id"))]
            )
            return self.create_multi_from_order_lines(
                orderlines=all_lines.ids, orders=orders
            )

    def create_multi_from_order_lines(self, orderlines=None, orders=None):
        return self.sudo().cmfol(orderlines=orderlines)

    def cmfol(self, orderlines=None):
        sol_obj = self.env["sale.order.line"]
        olines = sol_obj.browse(orderlines or [])
        lines = []
        for ol in olines:
    ...
    def _prepare_default_vals_copy(self, ol, ad_iss):
        ...
        res = {
            ...
            "from_date": ol.date_type == "issue_date"
            and ad_issue.issue_date
            or ol.from_date,
            "to_date": ol.date_type == "issue_date"
            and ad_issue.issue_date
            or ol.to_date,
        }
