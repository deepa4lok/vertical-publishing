...
class UpdateSOL(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if (
            ...
            and self._context.get("active_model") == "sale.advertising.issue"
        ):
            AdIssue = self.env["sale.advertising.issue"].browse(
            ...
            result.update(
                {
                    "issue_id": AdIssue.id,
                    "line_ids": [(6, 0, SOL.ids)],
                    "issue_date": AdIssue.issue_date,
                }
            )
        return result
