...
class SaleOrderLine(models.Model):
    ...
    ad_class_digital = fields.Boolean(
        ...
        string="Advertising Class Digital",
        default=False,
        store=True,
    )

    adv_class_issue_ids = fields.Many2many(
        "sale.advertising.issue",
        compute="_compute_class_issue_matrix",
    ...
    def _compute_class_issue_matrix(self):
        for ol in self:
            ...
            ol.adv_class_issue_ids = (
                self.env["sale.advertising.issue"].search(domain).ids
            )
