...
class AdOrderLineMakeInvoice(models.TransientModel):
    ...
    def modify_key(self, key, keydict, line):
        key, keydict = super(AdOrderLineMakeInvoice, self).modify_key(
            key, keydict, line
        )
        key = list(key)
    ...
    def make_invoices_job_queue(self, inv_date, post_date, chunk):
        """
        Filter out lines with invoicing properties pay in terms
        or invoice as package deal
        """
        ctx = self._context
        dropids = []
        if not ctx.get("invoice_from_order"):
            for line in chunk:
                if (
                    line.invoicing_property_id.inv_package_deal
                    or line.invoicing_property_id.pay_in_terms
        ...
        if not chunk:
            raise UserError(
                _(
                    "Only order lines are selected which should be invoiced at order level."
                )
            )
        return super(AdOrderLineMakeInvoice, self).make_invoices_job_queue(
            inv_date, post_date, chunk
        )
