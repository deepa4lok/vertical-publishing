...
class SaleOrder(models.Model):
    ...
    invoicing_property_id = fields.Many2one(
        "invoicing.property", string="Invoicing Property", required=True
    )
    invoicing_date = fields.Date()
    inv_date_bool = fields.Boolean(
        string="Set attribute to Invoicing date field",
        compute="_compute_helper_booleans",
        store=True,
    )
    inv_package_bool = fields.Boolean(string="Set attribute to Package")
    terms_condition = fields.Text(string="Description of terms")
    terms_cond_bool = fields.Boolean(
        string="Set attribute to Terms & condition field",
        compute="_compute_helper_booleans",
        store=True,
    )
        ...
        for line in self:
            if line.advertising_agency:
                if line.advertising_agency.invoicing_property_id:
                    line.invoicing_property_id = (
                        line.advertising_agency.invoicing_property_id.id
                    )
            else:
                if line.published_customer.invoicing_property_id:
                    line.invoicing_property_id = (
                        line.published_customer.invoicing_property_id.id
                    )

    @api.depends("invoicing_property_id")
    def _compute_helper_booleans(self):
        if (
            self.invoicing_property_id.inv_package_deal
            and self.invoicing_property_id.pay_in_terms
        ):
            self.update({"inv_date_bool": False})
            self.update({"package": True})
            self.update({"terms_cond_bool": True})
        elif (
            self.invoicing_property_id.pay_in_terms
            and not self.invoicing_property_id.inv_package_deal
        ):
            self.update({"terms_cond_bool": True})
            self.update({"inv_date_bool": False})
            ...
            self.invoicing_property_id.inv_package_deal
            and not self.invoicing_property_id.pay_in_terms
        ):
            ...
            self.invoicing_property_id.inv_per_line_adv_print
            or self.invoicing_property_id.inv_per_line_adv_online
            or self.invoicing_property_id.inv_whole_order_at_once
        ):
            self.update({"inv_date_bool": True})
        else:
            self.update({"inv_date_bool": False})
...
class SaleOrderLine(models.Model):
    ...
    invoicing_property_id = fields.Many2one(
        "invoicing.property",
        related="order_id.invoicing_property_id",
    ...
    @api.depends(
        "order_id.invoicing_property_id",
        "order_id.invoicing_date",
        "product_id",
        "issue_date",
        "from_date",
        "to_date",
    ...
    def _compute_cutoff_date(self):
        ...
        for line in self:
            ...
            if (
                line.invoicing_property_id.inv_whole_order_at_once
